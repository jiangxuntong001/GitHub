...
@property (nonatomic,assign)TFDropDownMenuTitleStyle menuTitleStyle;
@end
...
/**
 ...
 @return 实例对象
 */
- (instancetype)initWithFrame:(CGRect)frame firstArray:(NSMutableArray *)firstArray secondArray:(NSMutableArray *)secondArray MenuTitleStyle:(TFDropDownMenuTitleStyle )menuTitleStyle{
    ...
    if (self) {
        ...
        self.menuTitleStyle = menuTitleStyle;
        [self addAllSubView];
        [self addAction];
    }
    return self;
...
//         CGPoint titleLayerPosition = CGPointMake((i + 0.5) * backgroundLayerWidth, self.bounds.size.height * 0.5);
        CGFloat titleLX;
        if (self.menuTitleStyle == TFDropDownMenuTitleStyleLeft)
            titleLX = i + 0.1;
        else
            titleLX = i;
        CGPoint titleLayerPosition = CGPointMake(titleLX * backgroundLayerWidth, self.bounds.size.height * 0.5);
        ...
        CGFloat indicatorX;
        if (self.menuTitleStyle == TFDropDownMenuTitleStyleLeft)
            indicatorX = 0.95 * backgroundLayerWidth;
        else
             indicatorX = titleLayerPosition.x + (textSize.width / 2) + 10;
        
        CGPoint indicatorLayerPosition = CGPointMake(indicatorX, self.bounds.size.height * 0.5 + 2);
...
- (void)animateForTitleLayer:(CATextLayer *)textLayer indicator:(CAShapeLayer *)indicator show:(BOOL)show complete:(void(^)(void))complete {
    ...
    CGFloat indicatorX;
    if (self.menuTitleStyle == TFDropDownMenuTitleStyleLeft)
        indicatorX = 0.95 * self.bounds.size.width;
    else
        indicatorX = textLayer.position.x + textLayerWidth / 2 + 10;
    
    if (indicator) {
        indicator.position = CGPointMake(indicatorX, indicator.position.y) ;
    }
    if (show) {
